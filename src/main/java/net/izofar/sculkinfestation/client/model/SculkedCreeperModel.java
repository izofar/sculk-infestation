...
public class SculkedCreeperModel extends SinglePartEntityModel<SculkedCreeper> implements PartedModel{
    ...
    @Override
    public void setAngles(SculkedCreeper sculkedCreeper, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch){
        ...
        float k = animationProgress - (float) sculkedCreeper.age;
        this.setTendrilPitches(sculkedCreeper, animationProgress, k);
    }

    private void setTendrilPitches(SculkedCreeper sculkedCreeper, float animationProgress, float tickDelta) {
        float f;
        this.leftTendril.pitch = f = sculkedCreeper.getTendrilPitch(tickDelta) * (float)(Math.cos((double)animationProgress * 2.25) * Math.PI * (double)0.06f);
